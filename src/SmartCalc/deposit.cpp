...
void Deposit::getDeposit() {
  ...
  if (depositPeriodIndex == 0) {
    deposits = generateDeposits(0, 1, months);
  } else if (depositPeriodIndex == 1) {
    deposits = generateDeposits(ui->depositLine->text().toDouble(), 1, months);
  } else if (depositPeriodIndex == 2) {
    deposits = generateDeposits(ui->depositLine->text().toDouble(), 6, months);
  } else {
    deposits = generateDeposits(ui->depositLine->text().toDouble(), 12, months);
  }

  if (withdrawalsPeriodIndex == 0) {
    withdrawals = generateDeposits(0, 1, months);
  } else if (withdrawalsPeriodIndex == 1) {
    withdrawals =
        generateDeposits(ui->withdrawalsLine->text().toDouble(), 1, months);
  } else if (withdrawalsPeriodIndex == 2) {
    withdrawals =
        generateDeposits(ui->withdrawalsLine->text().toDouble(), 6, months);
  } else {
    withdrawals =
        generateDeposits(ui->withdrawalsLine->text().toDouble(), 12, months);
  }
